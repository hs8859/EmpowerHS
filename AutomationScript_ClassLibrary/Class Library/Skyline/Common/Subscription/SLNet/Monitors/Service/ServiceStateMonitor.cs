namespace Skyline.DataMiner.Library.Common.Subscription.Monitors
{
    ...
    internal class ServiceStateMonitor : Monitor
	{
		...
		internal ServiceStateMonitor(ICommunication connection, string sourceId, Service selection, string handleId) : base(connection, sourceId)
		{
			Initialize(selection, handleId);
		}

		internal ServiceStateMonitor(ICommunication connection, Element sourceElement, Service selection, string handleId) : base(connection, sourceElement)
		{
			Initialize(selection, handleId);
		}

		internal ServiceStateMonitor(ICommunication connection, Element sourceElement, Service selection) : this(connection, sourceElement, selection, "-ServiceState")
		{
		}

		internal ServiceStateMonitor(ICommunication connection, string sourceId, Service selection) : this(connection, sourceId, selection, "-ServiceState")
		{
		}
		...
		internal void Start(Action<ServiceStateChange> actionOnChange)
		{
			...
			else
			{
				...
				TryAddDestinationServiceCleanup(agentId, serviceId);
			}
		...
		private static Common.IServiceState ParseSlnetServiceState(ServiceStateEventMessage serviceStateMessage)
		{
			return new ServiceState(serviceStateMessage);
		}
		...
		private void HandleMatchingEvent(object sender, string guid, ServiceStateEventMessage serviceStateMessage)
		{
			...
			Common.IServiceState nonSLNetState = ParseSlnetServiceState(serviceStateMessage);

			System.Diagnostics.Debug.WriteLine("executing action...");

			var changed = new ServiceStateChange(new Service(serviceStateMessage.DataMinerID, serviceStateMessage.ElementID), SourceIdentifier, new Dms(com), nonSLNetState);
