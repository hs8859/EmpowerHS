			...
			foreach (ConnectionSetting property in this.ChangedPropertyList)
			{
				switch (property)
				{
					case ConnectionSetting.SecurityLevelAndProtocol:
						portInfo.StopBits = SnmpV3SecurityLevelAndProtocolAdapter.ToSLNetStopBits(this.securityLevelAndProtocol);
						break;
					case ConnectionSetting.EncryptionAlgorithm:
						portInfo.FlowControl = SnmpV3EncryptionAlgorithmAdapter.ToSLNetFlowControl(this.encryptionAlgorithm);
						break;
					case ConnectionSetting.AuthenticationProtocol:
						portInfo.Parity = SnmpV3AuthenticationAlgorithmAdapter.ToSLNetParity(this.authenticationAlgorithm);
						break;
